...
选用博图V18仿真软件，采用西门子plc的S7-1200，1214C AC/DC型CPU，使用Factory IO软件中的模型搭建工作站，并联合进行仿真，实现可视化调试。
编程语言为SCL语言与梯形图相结合。实现气动机械手的手动、自动模式切换；单周期，循环档位选择，紧急停车与复位等。
...
在整个控制流程设计中，对系统进行分析并分块实现，将底层逻辑的实现放置在SCL程序块中，每个板块之间的关系由梯形图呈现，更为直观，可读性强。
